...
def c3_detection(df: pd.DataFrame) -> pd.DataFrame:
    ...
    c3_stats['c3_port_diversity_ratio'] = c3_stats['c3_dst_port_diversity'] / \
        (c3_stats['c3_flow_count'] + 1)
    # extra feature 2: how the ratio failed/successfull connection is
    c3_stats['c3_failure_ratio'] = c3_stats['c3_failed_conns'] / \
        (c3_stats['c3_successful_conns'] + 1)
    return c3_stats
...
def c4_detection(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['is_big_payload'] = df['octetTotalCount'] > df['octetTotalCount'].quantile(
        0.75)
    ...
    c4_stats['c4_long_ratio_src'] = c4_stats['c4_long_flows_from_src'] / \
        (c4_stats['c4_flows_from_src'] + 1)
    # extra feature 2: how many flows very big packet flows from this source
    c4_stats['c4_big_payload_ratio_src'] = c4_stats['c4_big_payload_flows_from_src'] / \
        (c4_stats['c4_flows_from_src'] + 1)
...
def agg_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.merge(c1_features, left_on='dst_ip_int',
                  right_index=True, how='left')
    df = df.merge(c2_features, left_on='src_ip_int',
                  right_index=True, how='left')
    ...
    df['total_packets_from_src'] = df['total_packets_from_src'].fillna(
        df['packetTotalCount'])
    df['total_bytes_from_src'] = df['total_bytes_from_src'].fillna(
        df['octetTotalCount'])
    df['total_packets_to_dst'] = df['total_packets_to_dst'].fillna(
        df['packetTotalCount'])
    df['total_bytes_to_dst'] = df['total_bytes_to_dst'].fillna(
        df['octetTotalCount'])
